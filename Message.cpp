...
const vector<string> MESSAGE_STRINGS = {
        "board",
        "damage",
        "energy",
        "power",
...
Message::Message(const string& s) {
    ...
    auto it = find(MESSAGE_STRINGS.begin(), MESSAGE_STRINGS.end(), cmd_args.at(0));
...
string Message::actionWait() {
    return "wait";
}

string Message::actionRadar() {
    return "radar";
}
...
void testMessage() {
    ...
    transform(msg1.robots.begin(), msg1.robots.end(), robots.begin(), [](Direction d) -> Position { return Position() + d; });
