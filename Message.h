...
public:
    ...
    /**
     * Returns a vector of UpdateBoard messages, one for each roboter in the 'robots' parameter.
     * @param robots all alive robots on the field
     * @return a vector of UpdateBoard messages that can be sent to the robots.
     */
    static std::vector<std::string> updateBoard(const std::vector<Position>& robots);
