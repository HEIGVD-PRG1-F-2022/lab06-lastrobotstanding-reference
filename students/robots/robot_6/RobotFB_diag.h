...
class RobotFB_diag : public Robot {
    ...
    friend void traitementAction(std::vector<std::string> &updates, int &posRX, int &posRY, int &flagR, int &posBX, int &posBY, int &flagB,std::vector<std::vector<std::string>> &alentour);
    friend std::string realisationAction(const int &flagR, const int &posRX, const int &posRY, int &posBX, int &posBY, const int &flagB, size_t energy);
